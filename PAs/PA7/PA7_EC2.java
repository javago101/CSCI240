 ...
 * 
 * This program analyzes the relationship between Load Factor and Probe count
 * by testing a 100k data set against fixed table sizes (N/LF).
 */
public class PA7_EC2 {
    ...
    private static class SimpleChainHashMap {
        ...
        public void put(int key) {
            ...
            // Probe = 1 (bucket access) + current size (chain scan)
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=== PA7 Extra Credit Option 2: Chain Hashing Analysis (Controlled Variables) ===");
        ...
        // --- IMPORTANT: Global Hash Parameters ---
        // We use the same hash function for all tests to ensure the ONLY variable is Capacity.
        Random rand = new Random(2024); // Fixed seed for assignment consistency
        long globalScale = rand.nextInt(109345121 - 1) + 1;
        long globalShift = rand.nextInt(109345121);

        double[] factors = {0.25, 0.5, 0.75, 0.9};

        for (double lf : factors) {
            int capacity = (int) Math.ceil(N / lf);
            SimpleChainHashMap map = new SimpleChainHashMap(capacity, globalScale, globalShift);
            ...
            System.out.println("Bucket Count (N/LF) : " + capacity);
            System.out.printf("Average Probes      : %.4f%n", map.getAverageProbes(N));
            System.out.println("Max Probes (Worst)  : " + map.getMaxProbes());
        }
